...
def show_masha_easter_egg():
    """Показывает пасхалку после выполнения квеста Маши"""
    easter_window = tk.Toplevel(root)
    easter_window.title("🎉 СЕКРЕТ ОТКРЫТ! 🎉")
    easter_window.geometry("600x500")
    easter_window.configure(bg="black")
    
    # Делаем окно поверх всех
    easter_window.attributes('-topmost', True)
    
    # Анимация появления
    for i in range(5):
        easter_window.attributes('-alpha', i * 0.2)
        easter_window.update()
        time.sleep(0.05)
    
    # Заголовок с анимацией
    title_frame = tk.Frame(easter_window, bg="black")
    title_frame.pack(pady=20)
    
    title_label = tk.Label(title_frame, text="🎁 ПАСХАЛКА НАЙДЕНА! 🎁", 
                          font=("Arial", 20, "bold"), 
                          fg="gold", bg="black")
    title_label.pack()
    ...
    def blink_title():
        colors = ["gold", "red", "blue", "green", "purple", "cyan"]
        for color in colors:
            title_label.config(fg=color)
            easter_window.update()
            time.sleep(0.3)
        title_label.config(fg="gold")
    
    # Запускаем анимацию в отдельном потоке
    import threading
    blink_thread = threading.Thread(target=blink_title)
    blink_thread.daemon = True
    blink_thread.start()
    
    # Изображение Маши (с помощью emoji)
    masha_frame = tk.Frame(easter_window, bg="black")
    masha_frame.pack(pady=10)
    
    masha_emoji = tk.Label(masha_frame, text="👩‍🔬💕", 
                          font=("TkDefaultFont", 60),
                          bg="black", fg="pink")
    masha_emoji.pack()
    
    # Текст пасхалки
    text_frame = tk.Frame(easter_window, bg="black")
    text_frame.pack(pady=20, padx=20)
    
    easter_text = """✨ ТАЙНОЕ ПОСЛАНИЕ ОТ МАШИ ✨

Привет, сталкер!

Спасибо за помощь с артефактом! 
Ты настоящий герой Зоны!

Я хочу открыть тебе небольшой секрет...
Тот артефакт, что ты мне принес, 
был нужен для особого эксперимента.

Оказывается, Артемида содержит 
микроскопические частицы...
которые могут НЕЙТРАЛИЗОВАТЬ радиацию!

С помощью твоего артефакта я смогу 
создать прототип устройства, которое 
сделает Зону безопаснее для всех сталкеров.

Ты не просто заработал $300...
Ты сделал первый шаг к спасению Зоны!
...
"""
    ...
    text_widget.insert("1.0", easter_text)
    ...
    def typewriter_effect():
        ...
        full_text = easter_text
        for i in range(len(full_text) + 1):
            text_widget.delete("1.0", "end")
            text_widget.insert("1.0", full_text[:i])
            easter_window.update()
            time.sleep(0.02)
        text_widget.config(state="disabled")
    
    # Запускаем эффект печати
    type_thread = threading.Thread(target=typewriter_effect)
    type_thread.daemon = True
    type_thread.start()
    
    # Секретный бонус
    bonus_frame = tk.Frame(easter_window, bg="black")
    bonus_frame.pack(pady=10)
    
    tk.Label(bonus_frame, text="🎯 СЕКРЕТНЫЙ БОНУС:", 
            font=("Arial", 12, "bold"),
            fg="gold", bg="black").pack()
    
    tk.Label(bonus_frame, text="+50 XP за открытие пасхалки!", 
            font=("Arial", 10),
    ...
    # Секретная кнопка
    secret_button = tk.Button(easter_window, text="🔓 НАЖМИ МЕНЯ ДЛЯ СЕКРЕТА",
                             font=("Arial", 10, "bold"),
                             bg="purple", fg="white",
                             command=lambda: show_secret_reward(easter_window))
    ...
    def close_easter():
        ...
        messagebox.showinfo("Секрет сохранен", 
                          "Пасхалка сохранена в вашем профиле!\n"
                          "Вы можете найти её в журнале квестов рядом с выполненным квестом Маши.")
    
    tk.Button(easter_window, text="Закрыть", 
             command=close_easter,
             bg="red", fg="white",
             font=("Arial", 10, "bold")).pack(pady=10)

def show_secret_reward(parent_window):
    """Показывает секретную награду"""
    secret_window = tk.Toplevel(parent_window)
    secret_window.title("💎 СЕКРЕТНАЯ НАГРАДА 💎")
    secret_window.geometry("400x300")
    secret_window.configure(bg="darkblue")
    
    tk.Label(secret_window, text="🎊 ПОЗДРАВЛЯЮ! 🎊", 
            font=("Arial", 16, "bold"),
            fg="gold", bg="darkblue").pack(pady=20)
    
    tk.Label(secret_window, text="👑 ТЫ НАШЁЛ СУПЕР-СЕКРЕТ! 👑", 
            font=("Arial", 12),
            fg="cyan", bg="darkblue").pack(pady=10)
    
    # Специальная награда
    reward_frame = tk.Frame(secret_window, bg="darkblue")
    reward_frame.pack(pady=20)
    
    tk.Label(reward_frame, text="🎁 ТВОЯ НАГРАДА:", 
            font=("Arial", 12, "bold"),
            fg="white", bg="darkblue").pack()
    
    tk.Label(reward_frame, text="• +$100 к балансу", 
            font=("Arial", 10),
            fg="lightgreen", bg="darkblue").pack()
    
    tk.Label(reward_frame, text="• Легендарный артефакт 'Сердце Зоны'", 
            font=("Arial", 10),
            fg="gold", bg="darkblue").pack()
    
    tk.Label(reward_frame, text="• Уникальный титул 'Друг Маши'", 
            font=("Arial", 10),
            fg="pink", bg="darkblue").pack()
    
    # Выдаем награды
    global player_money, player_inventory
    player_money += 100
    player_inventory.append("Сердце Зоны (легендарный)")
    
    # Добавляем уникальный предмет в цены
    if "Сердце Зоны (легендарный)" not in item_prices:
        item_prices["Сердце Зоны (легендарный)"] = 5000
    
    tk.Button(secret_window, text="Забрать награду", 
             command=lambda: [secret_window.destroy(), 
                            messagebox.showinfo("Награда получена!", 
                                              "Вы получили:\n"
                                              "+$100\n"
                                              "Сердце Зоны (легендарный)\n"
                                              "Титул: Друг Маши"),
                            update_display()],
             bg="gold", fg="black",
             font=("Arial", 10, "bold")).pack(pady=20)

def show_all_easter_eggs():
    """Показывает все найденные пасхалки"""
    egg_window = tk.Toplevel(root)
    egg_window.title("🎪 КОЛЛЕКЦИЯ ПАСХАЛОК 🎪")
    egg_window.geometry("500x600")
    egg_window.configure(bg="#1A1A2E")
    
    # Заголовок
    tk.Label(egg_window, text="КОЛЛЕКЦИЯ СЕКРЕТОВ", 
            font=("Arial", 18, "bold"),
            bg="#1A1A2E", fg="gold").pack(pady=20)
    
    # Фрейм для пасхалок
    eggs_frame = tk.Frame(egg_window, bg="#1A1A2E")
    eggs_frame.pack(fill="both", expand=True, padx=20, pady=10)
    
    # Пасхалка Маши
    if easter_egg_found:
        masha_egg_frame = tk.Frame(eggs_frame, bg="#16213E", relief="ridge", bd=3)
        masha_egg_frame.pack(fill="x", pady=10)
        
        tk.Label(masha_egg_frame, text="👩‍🔬 ПАСХАЛКА МАШИ", 
                font=("Arial", 12, "bold"),
                bg="#16213E", fg="pink").pack(pady=10)
        
        tk.Label(masha_egg_frame, text="Статус: ✅ НАЙДЕНО", 
                font=("Arial", 10),
                bg="#16213E", fg="lightgreen").pack()
        
        tk.Label(masha_egg_frame, text="Открыто после выполнения квеста Маши", 
                font=("Arial", 9),
                bg="#16213E", fg="#CCCCCC").pack(pady=5)
        
        tk.Button(masha_egg_frame, text="Показать снова", 
                 command=show_masha_easter_egg,
                 bg="#9C27B0", fg="white",
                 font=("Arial", 9)).pack(pady=10)
    else:
        locked_frame = tk.Frame(eggs_frame, bg="#222222", relief="ridge", bd=3)
        locked_frame.pack(fill="x", pady=10)
        
        tk.Label(locked_frame, text="🔒 СЕКРЕТ МАШИ", 
                font=("Arial", 12, "bold"),
                bg="#222222", fg="#666666").pack(pady=10)
        
        tk.Label(locked_frame, text="Статус: ❌ НЕ НАЙДЕНО", 
                font=("Arial", 10),
                bg="#222222", fg="#FF5555").pack()
        
        tk.Label(locked_frame, text="Выполните квест Маши, чтобы открыть", 
                font=("Arial", 9),
                bg="#222222", fg="#888888").pack(pady=5)
    
    # Другие пасхалки (можно добавить в будущем)
    future_frame = tk.Frame(eggs_frame, bg="#0F3460", relief="ridge", bd=3)
    future_frame.pack(fill="x", pady=10)
    
    tk.Label(future_frame, text="🚧 БУДУЩИЕ СЕКРЕТЫ", 
            font=("Arial", 12, "bold"),
            bg="#0F3460", fg="#FFD700").pack(pady=10)
    
    tk.Label(future_frame, text="Больше пасхалок в следующих обновлениях!", 
            font=("Arial", 9),
            bg="#0F3460", fg="#CCCCCC").pack(pady=5)
    
    # Информация о прогрессе
    progress_frame = tk.Frame(egg_window, bg="#1A1A2E")
    progress_frame.pack(pady=20)
    
    eggs_found = 1 if easter_egg_found else 0
    total_eggs = 1  # Пока только одна пасхалка
    
    tk.Label(progress_frame, text=f"Найдено пасхалок: {eggs_found}/{total_eggs}", 
            font=("Arial", 11, "bold"),
            bg="#1A1A2E", fg="white").pack()
    
    # Прогресс-бар
    progress_canvas = tk.Canvas(progress_frame, width=300, height=20, bg="#1A1A2E", highlightthickness=0)
    progress_canvas.pack(pady=10)
    
    # Рисуем прогресс-бар
    progress_width = (eggs_found / total_eggs) * 300 if total_eggs > 0 else 0
    progress_canvas.create_rectangle(0, 0, progress_width, 20, fill="#4CAF50", outline="")
    progress_canvas.create_rectangle(0, 0, 300, 20, outline="white", width=2)
    
    tk.Button(egg_window, text="Закрыть", command=egg_window.destroy,
...
def complete_quest(quest_id):
    ...
    global player_money, player_inventory, quests, easter_egg_found
    ...
    if quest_id in quests:
        ...
        if quest["active"] and not quest["completed"]:
            ...
            if quest["required_item"] in player_inventory:
                ...
                # Активируем пасхалку для квеста Маши
                if quest_id == "masha_quest":
                    easter_egg_found = True
                    # Показываем пасхалку сразу после выполнения
                    show_masha_easter_egg()
...
def show_quest_log():
    """Показывает журнал квестов с пасхалкой"""
    quest_window = tk.Toplevel(root)
    quest_window.title("Журнал квестов")
    quest_window.geometry("550x450")  # Немного увеличим окно
    quest_window.configure(bg="#2C2C2C")
    
    # Заголовок
    title_frame = tk.Frame(quest_window, bg="#2C2C2C")
    title_frame.pack(fill="x", padx=20, pady=10)
    
    title_label = tk.Label(title_frame, text="ЖУРНАЛ КВЕСТОВ", 
                          font=("Arial", 16, "bold"), 
                          bg="#2C2C2C", fg="white")
    title_label.pack(side="left")
    
    # Кнопка секретов, если есть пасхалки
    if easter_egg_found:
        secret_btn = tk.Button(title_frame, text="🔍 Показать секреты", 
                              command=show_all_easter_eggs,
                              bg="#9C27B0", fg="white",
    ...
    for quest_id, quest in quests.items():
        ...
        # Кнопка пасхалки для выполненного квеста Маши
        if quest_id == "masha_quest" and quest["completed"] and easter_egg_found:
            egg_button = tk.Button(header_frame, text="🎁 Пасхалка",
                                  command=show_masha_easter_egg,
                                  bg="#FFD700", fg="black",
                                  font=("TkDefaultFont", 8, "bold"),
                                  relief="raised", bd=2)
            egg_button.pack(side="right", padx=5)
    ...
    # Показываем информацию о пасхалках
    if easter_egg_found:
        egg_info = tk.Label(stats_frame, 
                          text="🎉 Пасхалка найдена! Проверьте квест Маши!",
                          font=("TkDefaultFont", 10, "bold"),
                          bg="#2C2C2C", fg="#FFD700")
        egg_info.pack(pady=5)
...
def save_game(slot=1):
    ...
    save_data = {
        ...
        'easter_egg_found': easter_egg_found,  # Сохраняем пасхалку
...
def load_game(slot=1):
    ...
    global quick_access_slots, compass_active, visible_artifacts, quests, easter_egg_found
    ...
    try:
        ...
        # Восстанавливаем пасхалку
        if 'easter_egg_found' in save_data:
...
def settings_window():
    ...
    # Пасхалки
    if easter_egg_found:
        easter_frame = tk.Frame(settings_root, bg="#2C2C2C")
        easter_frame.pack(pady=10)
        
        tk.Button(easter_frame, text="🎪 Коллекция пасхалок", command=show_all_easter_eggs,
                 bg="#FFD700", fg="black", font=("TkDefaultFont", 10, "bold")).pack(pady=5, fill="x")
...
root = tk.Tk()
root.title("STALKER: Чернобыль - Квест Маши с Пасхалкой!")
...
if easter_egg_found:
    game_menu.add_separator()
    game_menu.add_command(label="🎪 Коллекция пасхалок", command=show_all_easter_eggs)
...
help_menu.add_command(label="Управление", 
                     ...
                     "F11 - Полный экран\n\n"
                     "🎁 СЕКРЕТНАЯ ПАСХАЛКА:\n"
                     "Выполните квест Маши и получите доступ\n"
                     "к секретной пасхалке в журнале квестов!\n\n"
...
messagebox.showinfo("Добро пожаловать!", 
                   ...
                   "⚡ ВЕРСИЯ С МАЛЕНЬКОЙ КАРТОЙ И ПАСХАЛКОЙ ⚡\n\n"
                   "🎯 ЦЕЛЬ ИГРЫ:\n"
                   ...
                   "🎁 СЕКРЕТНАЯ ПАСХАЛКА:\n"
                   "После выполнения квеста Маши вы получите\n"
                   "доступ к секретной пасхалке!\n"
                   "Проверьте журнал квестов (L) после выполнения.\n\n"
                   "📌 КЛЮЧЕВЫЕ КООРДИНАТЫ:\n"
                   "• Вы начинаете: (5, 10) ⚓\n"
                   "• Маша: (10, 10) 👩\n"
                   "• Торговая база: (20, 10) 🏗\n"
